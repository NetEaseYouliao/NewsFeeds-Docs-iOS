...
function adjustProtocol(httpsOn,dataList) {

    if (httpsOn) {
        for (j = 0, len = dataList.length; j < len; j++) {
            var newsInfo = dataList[j];
...
function imageFrameWithIndex(index) {
    var index = index;
    var img = $('.content img').get(index);
...
NewsDetail.prototype = {
    ...
    bindEvent: function () {
        ...
        // 相关推荐曝光
        $('.related .item').on('inview', function (event, isInView) {
            console.log($('.related .item').index(this));
            var index = $('.related .item').index(this);
            self.handleEvent('relatedNewsExpose', {
                index: index,
            });
        });
    ...
    render: function () {
        ...
        if(showControl.showRelated&&relatedData&&relatedData.length){
            // 根据控制协议调整是否启用https
            adjustProtocol(httpsOn,relatedData);
            
            var html = template('realtedTMPL', relatedData);
            // console.log(html);
